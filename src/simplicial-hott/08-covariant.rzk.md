...
#def covariant-transport-𝕀-line-const
  ...
  := id-arr-covariant-transport ⌈𝕀⌉ j C cov-C u
```
...
#def covariant-transport-𝕀-line-op
  ...
  ( cov
    : let mod ᵒᵖ p := packed in
        ᵒᵖ (is-covariant-𝕀-arrow (equiv-ext-shape-family-fwd 𝕀 □¹ p)))
  ( l :♭ (𝕀 → ⌈𝕀⌉))
  : ( let mod ᵒᵖ p := packed in
        ᵒᵖ (equiv-ext-shape-family-fwd 𝕀 □¹ p (l 0₂))
          → ᵒᵖ (equiv-ext-shape-family-fwd 𝕀 □¹ p (l 1₂)))
  :=
    \ x →
      let mod ᵒᵖ p := packed in
      ...
      let mod ᵒᵖ x0 := x in
        mod ᵒᵖ (
          covariant-transport-𝕀-line
            ( equiv-ext-shape-family-fwd 𝕀 □¹ p)
            ( cov0)
            ( l)
            ( x0))
```
