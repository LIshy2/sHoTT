...
The shape type former is replaced by inductive realizations: `Shape I φ` is
`⌈{t : I | φ}⌉`, freely generated by its points, with elimination by `match`
(there is no `unform`; a realization is write-only, and `match` binds the
point together with its tope).
...
#def equiv-shape-1-op-uninv
  ...
  :=
    -- TODO(migration): under the shape former, the ᵒᵖ-locked binder loaded
    -- its tope into the context; with realizations the tope enters by
    -- `match`, which here has to happen under the ᵒᵖ lock while the goal
    -- lives outside it. The equivalence needs restating (crisp parameter
    -- style) or an op-extraction lemma.
    ?todo-equiv-shape-1-op-uninv
...
#def eq-form-1-of-shape-at-1
  ...
  :=
    ( match t
        into ( \ t' → shape-at-1 t' → t' =_{⌈𝕀⌉} pt-⌈𝕀⌉ 1₂)
        ( point i ⇒ \ s' → match s' ( point u ⇒ refl)))
      ( s)
...
Maps `𝕀 → ⌈𝕀⌉` are monotone: if the point at `0` is `pt-⌈𝕀⌉ 1₂`, then every point on the line is `pt-⌈𝕀⌉ 1₂`.
